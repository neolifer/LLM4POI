...
# def manhattan_distance_simple(coord1, coord2):
...
#
#     # Converting latitude difference to distance in km
#     lat_dist = lat_diff * 111
#
#     # Average latitude for longitude distance calculation
#     avg_lat = (lat1 + lat2) / 2.0
#     # Converting longitude difference to distance in km
#     lon_dist = lon_diff * 111 * math.cos(math.radians(avg_lat))
#
#     # Summing the distances to get Manhattan distance
#     return int((lat_dist + lon_dist)*1000)
#
...
#
#             # Calculate the average coordinates for the current trajectory (excluding the last record)
#             avg_lat_long_current = user_trajectory_data.iloc[:-1][['Latitude', 'Longitude']].mean()
#
...
#                         (historical_data['pseudo_session_trajectory_id'].astype(str) in top200)].tail(600-num_traj)
#                 else:
#                     user_historical_data = main_data[
#                         (main_data['pseudo_session_trajectory_id'].astype(str) in top200)].tail(600-num_traj)
#             else:
#                 if historical_data is not None:
...
#             if not pd.isna(avg_lat_long_current['Longitude']):
#                 question_parts = [
#                     f"<question>: The following data is the current trajectory of user {user}:"]
#             user_trajectory_data.reset_index(drop=True, inplace=True)
#             for i, row in user_trajectory_data.iloc[:-1].iterrows():
#                 if i > 0:
#                     question_parts.append(
#                         f"At {row['UTCTimeOffset']}, user {user} visited POI id {row['PoiId']} which is a {row['PoiCategoryName']} and has Category id {row['PoiCategoryId']} with distance to the start {manhattan_distance_simple((Latitude,Longtitude),(row['Latitude'],row['Longitude']))}m.")
#                     continue
#                 else:
...
#
#             total_records = user_trajectory_data.shape[0] - 1  # Excluding the last entry of the current trajectory
#
#             if not user_historical_data.empty:
#                 question_parts.append("There are historical trajectories:")
#                 user_historical_data.sort_values(by=['UTCTimeOffsetEpoch'])
#                 for i, traj_id in enumerate(user_historical_data['pseudo_session_trajectory_id'].unique()):
#                     # Get historical trajectory data
#                     traj_data = user_historical_data[user_historical_data['pseudo_session_trajectory_id'] == traj_id]
#
#                     # Calculate the average coordinates for the historical trajectory
#                     avg_lat_long_hist = traj_data[['Latitude', 'Longitude']].mean()
#
#
#                     # Check if adding this trajectory exceeds the limit of 200 records
#                     if total_records > 150:
#                         break  # Stop adding more trajectories if the limit is reached
#                     if not pd.isna(avg_lat_long_current['Longitude']):
#                         question_parts.append(
#                             f"{i}.Trajectory with distance to current trajectory {manhattan_distance_simple((avg_lat_long_hist['Latitude'], avg_lat_long_hist['Longitude']),(avg_lat_long_current['Latitude'],avg_lat_long_current['Longitude']))}:")
#                     else:
#                         question_parts.append(
#                             f"{i}.Trajectory:")
#                     traj_data.reset_index(drop=True, inplace=True)
#                     for j, row in traj_data.iterrows():
#                         total_records += 1
#                         if j > 0:
#                             question_parts.append(
#                                 f"At {row['UTCTimeOffset']}, user {user} visited POI id {row['PoiId']} which is a {row['PoiCategoryName']} and has Category id {row['PoiCategoryId']} with distance to the start {manhattan_distance_simple((Latitude, Longtitude), (row['Latitude'], row['Longitude']))}m.")
...
#                 poi_num = 9689
#
#             question += f" Given the data, At {user_trajectory_data.iloc[-1]['UTCTimeOffset']}, Which POI id will user {user} visit next? Note that POI id is an integer in the range from 0 to {poi_num}."
#
    ...
    for user in tqdm(main_data['UserId'].unique()):
        ...
        for traj_id in user_data['pseudo_session_trajectory_id'].unique():
            ...
            if 'traj_id' in kqt.keys():
                ...
                # Fetch historical data before the start of the current trajectory
                if historical_data is not None:
                    user_historical_data = historical_data[(str(historical_data['pseudo_session_trajectory_id']) in top200)].tail(300 - num_traj)
                else:
                    user_historical_data = main_data[(str(main_data['pseudo_session_trajectory_id']) in top200)].tail(300 - num_traj)
            else:
                if historical_data is not None:
                    user_historical_data = historical_data[(historical_data['UserId'] == user) & (
                            historical_data['UTCTimeOffsetEpoch'] < start_time_of_current_traj)].tail(600 - num_traj)
            ...
            # Create the final question string
            question = " ".join(question_parts)
            question += f" Given the data, At {user_trajectory_data.iloc[-1]['UTCTimeOffset']}, Which POI id will user {user} visit? Note that POI id is an integer in the range from 0 to 9689."
